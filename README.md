[![Typing SVG](https://readme-typing-svg.herokuapp.com/?color=d5a6bd&size=35&center=true&vCenter=true&width=1000&lines=Bem-vindo(a)+ao+meu+GitHub+🩷;Estudante+de+ADS+%7C+Suporte+de+TI+%26+Cybersecurity;Construindo+projetos+e+laboratórios+💻;Sempre+aprendendo+🔐)](https://git.io/typing-svg)

<div align="center">

### 💗 Olá! Eu sou Evellyn Ellen

💻 Estudante de Análise e Desenvolvimento de Sistemas (5º semestre)

🔐 Focada em Suporte de TI, Infraestrutura e Cybersecurity

📚 Construindo laboratórios práticos e documentando tudo no GitHub

</div>

---

## 🛡️ Sobre Mim

<div align="center">

Sou estudante de **Análise e Desenvolvimento de Sistemas**, apaixonada por tecnologia, infraestrutura e segurança da informação.

Atualmente estou desenvolvendo uma base sólida em **Windows, Linux, Redes, Cloud e Cybersecurity**, criando laboratórios práticos para simular ambientes corporativos e documentando toda minha evolução no GitHub.

🎯 Meu objetivo é iniciar minha carreira em **Suporte de TI / Service Desk**, evoluindo para **Infraestrutura** e futuramente para **Cybersecurity**.

</div>

---

## 🚀 Projetos

> 📌 Em construção... cada estudo se transforma em um projeto documentado.

📂 **windows-troubleshooting-guide**  
Base de conhecimento com soluções para problemas comuns do Windows.

📂 **linux-server-labs**  
Laboratórios utilizando Ubuntu Server.

📂 **network-labs**  
Estudos de TCP/IP, DNS, DHCP, VLAN e Wireshark.

📂 **active-directory-home-lab**  
Empresa fictícia utilizando Windows Server e Active Directory.

📂 **powershell-scripts**  
Scripts para automação de tarefas administrativas.

📂 **cybersecurity-notes**  
Anotações, desafios e estudos sobre segurança da informação.

---

## 📚 Atualmente Estudando

| Área | Conteúdo |
|------|----------|
| 🖥️ Windows | Administração, PowerShell, Troubleshooting |
| 🐧 Linux | Terminal, Permissões, SSH, Serviços |
| 🌐 Redes | TCP/IP, DNS, DHCP, VLAN, VPN |
| ☁️ Cloud | Fundamentos AWS |
| 🔐 Cybersecurity | OWASP, Criptografia, Hashing |
| 🧪 Labs | VirtualBox, TryHackMe, Wireshark, Nmap |

---

## 🎯 Roadmap

- ✅ Windows
- ✅ Linux
- 🟡 Redes
- 🟡 PowerShell
- ⬜ Windows Server
- ⬜ Active Directory
- ⬜ AWS
- ⬜ Docker
- ⬜ TryHackMe
- ⬜ Security+

---

## 🛠️ Tech Stack

<div align="center">

<img src="https://img.shields.io/badge/Linux-D5A6BD?style=for-the-badge&logo=linux&logoColor=white"/>

<img src="https://img.shields.io/badge/Windows-0D1117?style=for-the-badge&logo=windows&logoColor=D5A6BD"/>

<img src="https://img.shields.io/badge/PowerShell-D5A6BD?style=for-the-badge&logo=powershell&logoColor=white"/>

<img src="https://img.shields.io/badge/Python-0D1117?style=for-the-badge&logo=python&logoColor=D5A6BD"/>

<img src="https://img.shields.io/badge/Git-D5A6BD?style=for-the-badge&logo=git&logoColor=white"/>

<img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=D5A6BD"/>

<img src="https://img.shields.io/badge/VirtualBox-D5A6BD?style=for-the-badge&logo=virtualbox&logoColor=white"/>

<img src="https://img.shields.io/badge/Wireshark-0D1117?style=for-the-badge&logo=wireshark&logoColor=D5A6BD"/>

<img src="https://img.shields.io/badge/Nmap-D5A6BD?style=for-the-badge&logo=gnuprivacyguard&logoColor=white"/>

<img src="https://img.shields.io/badge/AWS-0D1117?style=for-the-badge&logo=amazonaws&logoColor=D5A6BD"/>

</div>

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=evellynellen&show_icons=true&hide_border=true&bg_color=0d1117&title_color=d5a6bd&icon_color=d5a6bd&text_color=f8d7e3"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=evellynellen&layout=compact&hide_border=true&bg_color=0d1117&title_color=d5a6bd&text_color=f8d7e3"/>

<br><br>

<img src="https://streak-stats.demolab.com?user=evellynellen&hide_border=true&background=0D1117&ring=D5A6BD&fire=D5A6BD&currStreakNum=F8D7E3&sideNums=F8D7E3&currStreakLabel=D5A6BD&sideLabels=D5A6BD&dates=F8D7E3&stroke=D5A6BD"/>

</div>

---

## 🏆 Certificações

⌛ Google IT Support Professional Certificate

⌛ Cisco Networking Basics

⌛ AWS Cloud Practitioner

⌛ Microsoft AZ-900

⌛ CompTIA Security+

---

## 🌷 Contato

<div align="center">

<a href="mailto:ellenevellyn988@gmail.com">
<img src="https://img.shields.io/badge/Gmail-D5A6BD?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/evellyn-ellen/" target="_blank">
<img src="https://img.shields.io/badge/LinkedIn-D5A6BD?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

</div>

---

<div align="center">

### 💗 Aprendendo todos os dias, criando laboratórios e compartilhando conhecimento através do GitHub.

*"Transformando teoria em prática, um commit por vez."* ✨

</div>
